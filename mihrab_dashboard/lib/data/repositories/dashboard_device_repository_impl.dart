...
class DashboardDeviceRepositoryImpl implements DashboardDeviceRepository {
  final SupabaseClient _client;
  final _storage = GetStorage();

  static const _linkedDevicesKey = 'LINKED_DEVICE_IDS';

  DashboardDeviceRepositoryImpl(this._client);
  ...
  Future<void> deleteDevice(String deviceId) async {
    // Delete from Supabase (CASCADE will remove device_settings)
    await _client.from('devices').delete().eq('id', deviceId);

    // Remove from local linked IDs
    final ids = getLinkedDeviceIds();
    ids.remove(deviceId);
